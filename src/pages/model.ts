...
function applyFilters(
  ...
  if (geneSymbols.size > 0 && diagnosisList.size > 0) {
    return geneExpressions.filter((ge) => {
      return geneSymbols.has(ge.geneSymbol) && diagnosisList.has(ge.diagnosis);
    });
  }
 ...
 */
function aggregateAndAverageZscores(
  ...
  return expressions.reduce((acc: GeneExpression, expression, i) => {
    ...
    acc.zScore =
      lastItemIndex === i ? newScore / (lastItemIndex + 1) : newScore;
...
function sortByDiagnosis(arr: GeneExpression[], diagnosisList: string[]) {
  return arr.sort((a, b) => {
    return (
      diagnosisList.findIndex((value) => value === a?.diagnosis) -
      diagnosisList.findIndex((value) => value === b?.diagnosis)
    );
  });
}

function filterByzScorePercentage(
  arr: GeneExpression[],
  zScorePercentage: number,
) {
  const totalValue =
...
export default {
  ...
  state: initialState,
  reducers: {
    [GeneAction.SET_EXPRESSION_DATA](
      state: GeneState,
      { geneExpressions }: { geneExpressions: GeneExpression[] },
    ) {
      state.geneExpressions = geneExpressions;
    },
    [GeneAction.SET_FILTER_OPTIONS](
      state: GeneState,
      {
        geneSymbols,
        diagnosisList,
      }: { geneSymbols: string[]; diagnosisList: string[] },
    ) {
      state.filterOptions[Filters.GeneSymbols] = geneSymbols;
      state.filterOptions[Filters.DiagnosisList] = diagnosisList;
    },
    [GeneAction.SET_SELECTED_FILTERS](
      state: GeneState,
      { selectedFilter, value }: { selectedFilter: Filters; value: string[] },
    ) {
      state.selectedFilters[selectedFilter] = value;
    },
    [GeneAction.SET_ZSCORE_PERCENTAGE](
      state: GeneState,
      { zScorePercentage }: { zScorePercentage: number },
    ) {
      const { geneExpressions, filterOptions, selectedFilters } = state;
      const diagnosisList = filterOptions[Filters.DiagnosisList];

      if (!!geneExpressions) {
        const updateGeneExpressions = sortByDiagnosis(
          filterByzScorePercentage(geneExpressions, zScorePercentage),
          diagnosisList as string[],
        );
        state.zScorePercentage = zScorePercentage;
        state.filteredGeneExpressions = applyFilters(
          updateGeneExpressions,
          selectedFilters,
        );
      }
    },
  ...
  effects: {
    *[GeneAction.FETCH_EXPRESSION_DATA](
      _payload: any,
      ...
      try {
        ...
        if (geneExpressions?.length) {
          ...
          let groupedExpressions: GeneExpression[][] = Object.values(
            groupByModelIdAndGeneSymbol(geneExpressions),
          );
          // aggregating and taking average of all the values z-score value based on same model ids and gene symbols.
          geneExpressions = groupedExpressions.reduce(
            (acc, geneExpressions) => {
        ...
        const filterOptions = geneExpressions.reduce(
          (acc, geneExpression) => {
        ...
        yield put.resolve({
          ...
          geneExpressions: sortByDiagnosis(
            filterByzScorePercentage(geneExpressions, 100),
            diagnosisList as string[],
          ),
        });
      } catch (e) {
